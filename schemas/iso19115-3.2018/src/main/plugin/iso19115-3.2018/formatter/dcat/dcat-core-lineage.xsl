        ...
        <xsl:if test="$isExpandSkosConcept">
          ...
          <xsl:for-each select="$languages">
            <xsl:variable name="codelistTranslation"
                          select="tr:codelist-value-label(tr:create('iso19115-3.2018', ./@iso3code), $parentName, $codelistKey)"/>

            <skos:prefLabel xml:lang="{./@iso2code}"><xsl:value-of select="$codelistTranslation"/></skos:prefLabel>
          </xsl:for-each>
        </xsl:if>
      </skos:Concept>
    </adms:status>

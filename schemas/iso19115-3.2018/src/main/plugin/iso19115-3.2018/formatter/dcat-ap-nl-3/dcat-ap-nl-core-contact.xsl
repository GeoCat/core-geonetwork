...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template mode="iso19115-3-to-dcat"
    ...
    <xsl:variable name="dcatElementConfig">
      <value name="dct:creator" as="foaf"/>
      <value name="dct:publisher" as="foaf"/>
      <value name="dcat:contactPoint" as="vcard"/>
      <value name="dct:rightsHolder" as="foaf"/>
    </xsl:variable>
  ...
  <xsl:template name="rdf-contact-foaf">
    ...
    <xsl:message>
      ...
      isindividual: <xsl:value-of select="$isindividual" />
      individualName: <xsl:value-of select="$individualName" />
    </xsl:message>
    <rdf:Description>
      <xsl:call-template name="rdf-object-ref-attribute">
